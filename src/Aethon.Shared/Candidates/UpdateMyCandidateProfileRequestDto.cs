...
public sealed class UpdateMyCandidateProfileRequestDto : IValidatableObject
{
    ...
    /// <summary>
    /// Age classification. Must be set before the job seeker can apply for jobs.
    /// SchoolLeaver requires BirthMonth + BirthYear. Adult requires no date.
    /// </summary>
    public ApplicantAgeGroup AgeGroup { get; set; }

    /// <summary>Birth month (1–12). Required when AgeGroup = SchoolLeaver.</summary>
    [Range(1, 12)]
    public int? BirthMonth { get; set; }

    /// <summary>Birth year (e.g. 2009). Required when AgeGroup = SchoolLeaver.</summary>
    [Range(1900, 9999)]
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (AgeGroup == ApplicantAgeGroup.SchoolLeaver)
        {
            if (!BirthMonth.HasValue)
                yield return new ValidationResult(
                    "Birth month is required for school leaver accounts.",
                    [nameof(BirthMonth)]);

            if (!BirthYear.HasValue)
                yield return new ValidationResult(
                    "Birth year is required for school leaver accounts.",
                    [nameof(BirthYear)]);
        }

        if (AgeGroup == ApplicantAgeGroup.Adult && (BirthMonth.HasValue || BirthYear.HasValue))
        {
            yield return new ValidationResult(
                "Date of birth information must not be provided for adult accounts.",
                [nameof(BirthMonth)]);
        }
    }
}
