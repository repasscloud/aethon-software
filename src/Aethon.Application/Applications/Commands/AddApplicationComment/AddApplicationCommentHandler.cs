using Aethon.Application.Abstractions.Authentication;
using Aethon.Application.Abstractions.Time;
using Aethon.Application.Activity.Services;
using Aethon.Application.Applications.Services;
using Aethon.Application.Common.Results;
using Aethon.Data;
using Aethon.Data.Entities;
using Microsoft.EntityFrameworkCore;

namespace Aethon.Application.Applications.Commands.AddApplicationComment;

public sealed class AddApplicationCommentHandler
{
    private readonly AethonDbContext _dbContext;
    private readonly ICurrentUserAccessor _currentUserAccessor;
    private readonly IDateTimeProvider _dateTimeProvider;
    private readonly ApplicationAccessService _applicationAccessService;
    private readonly ActivityLogWriter _activityLogWriter;

    public AddApplicationCommentHandler(
        AethonDbContext dbContext,
        ICurrentUserAccessor currentUserAccessor,
        IDateTimeProvider dateTimeProvider,
        ApplicationAccessService applicationAccessService,
        ActivityLogWriter activityLogWriter)
    {
        _dbContext = dbContext;
        _currentUserAccessor = currentUserAccessor;
        _dateTimeProvider = dateTimeProvider;
        _applicationAccessService = applicationAccessService;
        _activityLogWriter = activityLogWriter;
    }

    public async Task<Result<AddApplicationCommentResult>> HandleAsync(
        AddApplicationCommentCommand command,
        CancellationToken cancellationToken = default)
    {
        if (!_currentUserAccessor.IsAuthenticated || _currentUserAccessor.UserId == Guid.Empty)
        {
            return Result<AddApplicationCommentResult>.Failure(
                "auth.unauthenticated",
                "The current user is not authenticated.");
        }

        var content = Normalize(command.Content);

        if (content is null)
        {
            return Result<AddApplicationCommentResult>.Failure(
                "applications.comment.content_required",
                "Comment content is required.");
        }

        var currentUserId = _currentUserAccessor.UserId;

        var application = await _dbContext.JobApplications
            .Include(x => x.Job)
            .SingleOrDefaultAsync(x => x.Id == command.ApplicationId, cancellationToken);

        if (application is null)
        {
            return Result<AddApplicationCommentResult>.Failure(
                "applications.not_found",
                "The requested application was not found.");
        }

        var canManage = await _applicationAccessService.CanManageApplicationAsync(
            currentUserId,
            command.ApplicationId,
            cancellationToken);

        if (!canManage)
        {
            return Result<AddApplicationCommentResult>.Failure(
                "applications.forbidden",
                "The current user cannot add comments to this application.");
        }

        if (command.ParentCommentId.HasValue)
        {
            var parentExists = await _dbContext.JobApplicationComments
                .AsNoTracking()
                .AnyAsync(
                    x => x.Id == command.ParentCommentId.Value &&
                         x.JobApplicationId == command.ApplicationId &&
                         !x.IsDeleted,
                    cancellationToken);

            if (!parentExists)
            {
                return Result<AddApplicationCommentResult>.Failure(
                    "applications.comment.parent_not_found",
                    "The parent comment was not found.");
            }
        }

        var utcNow = _dateTimeProvider.UtcNow;

        var comment = new JobApplicationComment
        {
            Id = Guid.NewGuid(),
            JobApplicationId = application.Id,
            ParentCommentId = command.ParentCommentId,
            Content = content,
            CreatedUtc = utcNow,
            CreatedByUserId = currentUserId
        };

        _dbContext.JobApplicationComments.Add(comment);

        application.LastActivityUtc = utcNow;
        application.UpdatedUtc = utcNow;
        application.UpdatedByUserId = currentUserId;

        await _activityLogWriter.WriteAsync(
            entityType: nameof(JobApplication),
            entityId: application.Id,
            action: command.ParentCommentId.HasValue ? "CommentReplyAdded" : "CommentAdded",
            performedUtc: utcNow,
            performedByUserId: currentUserId,
            organisationId: application.Job.OwnedByOrganisationId,
            summary: command.ParentCommentId.HasValue
                ? "Comment reply added to application."
                : "Comment added to application.",
            details: Truncate(content, 500),
            cancellationToken: cancellationToken);

        await _dbContext.SaveChangesAsync(cancellationToken);

        return Result<AddApplicationCommentResult>.Success(new AddApplicationCommentResult
        {
            Id = comment.Id,
            ApplicationId = comment.JobApplicationId,
            ParentCommentId = comment.ParentCommentId,
            Content = comment.Content,
            CreatedUtc = comment.CreatedUtc
        });
    }

    private static string? Normalize(string? value) =>
        string.IsNullOrWhiteSpace(value) ? null : value.Trim();

    private static string Truncate(string value, int maxLength) =>
        value.Length <= maxLength ? value : value[..maxLength];
}
