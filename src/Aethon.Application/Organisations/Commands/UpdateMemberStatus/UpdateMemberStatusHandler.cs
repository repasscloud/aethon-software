...
namespace Aethon.Application.Organisations.Commands.UpdateMemberStatus;

public sealed class UpdateMemberStatusHandler
{
    private readonly AethonDbContext _db;
    private readonly ICurrentUserAccessor _currentUser;

    public UpdateMemberStatusHandler(AethonDbContext db, ICurrentUserAccessor currentUser)
    {
        _db = db;
        _currentUser = currentUser;
    }

    public async Task<Result> HandleAsync(Guid targetUserId, UpdateMemberStatusRequestDto request, CancellationToken ct = default)
    {
        if (!_currentUser.IsAuthenticated)
            return Result.Failure("auth.unauthenticated", "Not authenticated.");

        if (!Enum.TryParse<MembershipStatus>(request.Status, ignoreCase: true, out var newStatus) ||
            newStatus == MembershipStatus.Pending)
        {
            return Result.Failure("organisations.member.invalid_status",
                "Invalid status. Allowed values: Active, Suspended, Revoked.");
        }
        ...
        if (!callerIsOwnerOrAdmin)
            return Result.Failure("organisations.forbidden", "Insufficient permissions to update member status.");
        ...
        if (target.IsOwner)
            return Result.Failure("organisations.forbidden", "The organisation owner's status cannot be changed.");

        // Cannot change your own status
        if (targetUserId == _currentUser.UserId)
            return Result.Failure("organisations.member.invalid", "You cannot change your own status.");

        target.Status = newStatus;
        if (newStatus == MembershipStatus.Revoked)
