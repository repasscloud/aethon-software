...
namespace Aethon.Application.RecruiterCompanies.Queries.GetRecruiterCompanies;

public sealed class GetRecruiterCompaniesHandler
{
    private readonly AethonDbContext _db;
    private readonly ICurrentUserAccessor _currentUser;

    public GetRecruiterCompaniesHandler(AethonDbContext db, ICurrentUserAccessor currentUser)
    {
        _db = db;
    ...
    public async Task<Result<List<RecruiterCompanyRelationshipDto>>> HandleAsync(CancellationToken ct = default)
    {
        ...
        if (myMembership is null)
            return Result<List<RecruiterCompanyRelationshipDto>>.Failure("organisations.not_found", "No active recruiter membership found.");
        ...
        var partnerships = await _db.OrganisationRecruitmentPartnerships
            ...
            .Where(p => p.RecruiterOrganisationId == recruiterOrgId)
