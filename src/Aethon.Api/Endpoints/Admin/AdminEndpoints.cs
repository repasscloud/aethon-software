...
public static class AdminEndpoints
{
    public static void MapAdminEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/compose-email", async (
            HttpContext http,
            AethonDbContext db,
            ...
            [FromServices] ActivityLogWriter activityLog,
            [FromBody] AdminComposeEmailRequest request,
            CancellationToken ct) =>
        {
            var senderEmail = http.User.FindFirstValue(ClaimTypes.Email) ?? "unknown";
            var senderIdStr = http.User.FindFirstValue(ClaimTypes.NameIdentifier);
            Guid.TryParse(senderIdStr, out var senderId);

            var opts = await emailOptionsResolver.ResolveAsync(ct);
            if (string.IsNullOrWhiteSpace(opts.FromEmailSupport))
            {
                await systemLog.WarnAsync("AdminEmail",
                    "Admin email not sent — Email__FromEmailSupport is not configured.",
                    $"Attempted by: {senderEmail}", ct);
                return Results.Problem(
                    detail: "Email__FromEmailSupport is not configured. Set it in Admin → Settings or via environment variable.",
                    statusCode: 503);
            }
            ...
            if (recipient is null)
                return Results.NotFound(new { code = "user.not_found", message = "User not found." });

            // Wrap body in the standard branded email layout
            var wrappedHtml = emailTemplates.WrapHtml(request.HtmlBody);

            // Strip tags for plain-text fallback
            var textBody = Regex.Replace(request.HtmlBody, "<[^>]+>", " ");
            textBody = Regex.Replace(textBody, @"\s{2,}", " ").Trim();
            ...
            await emailService.SendAsync(new EmailMessage
            {
                ...
                TextBody     = textBody,
                ReplyToEmail = opts.FromEmailSupport,
                ReplyToName  = opts.FromName
            }, ct);

            await systemLog.InfoAsync("AdminEmail",
                $"Admin email sent to {recipient.Email} by {senderEmail}",
                $"Subject: {request.Subject} | RecipientId: {recipient.Id}", ct);

            await activityLog.WriteAsync(
                entityType: "ApplicationUser",
                entityId: recipient.Id,
                action: "AdminEmailSent",
                performedUtc: DateTime.UtcNow,
                performedByUserId: senderId == Guid.Empty ? null : senderId,
                summary: $"Admin email sent: {request.Subject}",
                details: $"Sent by: {senderEmail}",
                cancellationToken: ct);
            await db.SaveChangesAsync(ct);

            return Results.Ok(new { sent = true, toEmail = recipient.Email, toName = recipient.DisplayName });
        });

        // ─── Account Deletion (Age Policy & Privacy Compliance) ──────────────────────

        // DELETE /api/v1/admin/job-seekers/{userId}/account — admin-triggered deletion
        group.MapDelete("/job-seekers/{userId:guid}/account", async (
            AethonDbContext db,
            UserManager<ApplicationUser> userManager,
            Guid userId,
            CancellationToken ct) =>
        {
            var user = await userManager.FindByIdAsync(userId.ToString());
            if (user is null)
                return Results.NotFound(new { code = "account.not_found", message = "Job seeker account not found." });

            if (user.UserType != UserAccountType.JobSeeker)
                return Results.BadRequest(new { code = "account.wrong_type", message = "This endpoint only deletes job seeker accounts." });

            await Aethon.Api.Endpoints.Candidates.CandidateEndpoints.DeleteJobSeekerAccountAsync(
                db, userManager, user, ct);

            return Results.Ok(new { message = $"Account {userId} and associated data have been deleted." });
        })
        ...
        // Purges school leaver accounts (AgeGroup = SchoolLeaver) inactive for 30+ days.
        group.MapPost("/maintenance/purge-inactive-school-leavers", async (
            AethonDbContext db,
            UserManager<ApplicationUser> userManager,
            CancellationToken ct) =>
        {
            ...
            // Find school leaver user IDs that have been inactive for 30+ days
            var inactiveUserIds = await db.JobSeekerProfiles
                .AsNoTracking()
                .Where(p => p.AgeGroup == ApplicantAgeGroup.SchoolLeaver)
                .Join(db.Users,
                    p => p.UserId,
                    u => u.Id,
                    (p, u) => new { u.Id, u.LastLoginUtc, u.UserType })
                .Where(x => x.UserType == UserAccountType.JobSeeker
                         && (x.LastLoginUtc == null || x.LastLoginUtc < cutoff))
                .Select(x => x.Id)
                .ToListAsync(ct);
            ...
            return Results.Ok(new { purged = deleted, cutoffUtc = cutoff });
        })
        .RequireAuthorization(policy => policy.RequireRole("SuperAdmin", "Admin"));

        // POST /api/v1/admin/maintenance/purge-inactive-adults
        // Purges adult job seeker accounts (AgeGroup = Adult) inactive for 90+ days
        // where the candidate has no successful (hired) applications.
        group.MapPost("/maintenance/purge-inactive-adults", async (
            AethonDbContext db,
            UserManager<ApplicationUser> userManager,
            CancellationToken ct) =>
        {
            ...
            // Find adult user IDs: inactive 90+ days AND have never been hired
            var hiredUserIds = await db.JobApplications
                .AsNoTracking()
                .Where(a => a.IsHired)
                .Select(a => a.UserId)
                .Distinct()
            ...
            var inactiveUserIds = await db.JobSeekerProfiles
                .AsNoTracking()
                .Where(p => p.AgeGroup == ApplicantAgeGroup.Adult
                         && !hiredUserIds.Contains(p.UserId))
                .Join(db.Users,
                    p => p.UserId,
                    u => u.Id,
                    (p, u) => new { u.Id, u.LastLoginUtc, u.UserType })
        ...
        // ── ATS match queue admin ─────────────────────────────────────────────────

        group.MapGet("/ats-queue", async (
            AethonDbContext db,
            AtsMatchStatus? status,
            AtsMatchProvider? provider,
            int page = 1,
            int pageSize = 50,
            CancellationToken ct = default) =>
        {
            ...
            var q = db.AtsMatchQueue.AsNoTracking();

            if (status.HasValue)    q = q.Where(x => x.Status   == status.Value);
            if (provider.HasValue)  q = q.Where(x => x.Provider == provider.Value);
        ...
        group.MapPost("/ats-queue/{id:guid}/retry", async (
            AethonDbContext db,
            Guid id,
            CancellationToken ct) =>
        {
            var item = await db.AtsMatchQueue.FirstOrDefaultAsync(x => x.Id == id, ct);
            if (item is null) return Results.NotFound();

            if (item.Status != AtsMatchStatus.Failed && item.Status != AtsMatchStatus.Processing)
                return Results.BadRequest(new { error = "Only Failed or stuck Processing items can be retried." });

            item.Status       = AtsMatchStatus.Pending;
            item.Attempts     = Math.Max(0, item.Attempts - 1); // Refund one attempt
            item.ErrorMessage = null;
            item.UpdatedUtc   = DateTime.UtcNow;

            await db.SaveChangesAsync(ct);
            return Results.Ok(new { id, status = item.Status.ToString() });
        })
        .RequireAuthorization(policy => policy.RequireRole("SuperAdmin", "Admin"));

        group.MapPost("/ats-queue/{id:guid}/promote", async (
            AethonDbContext db,
            Guid id,
            CancellationToken ct) =>
        {
            ...
            if (item.Status != AtsMatchStatus.Pending)
                return Results.BadRequest(new { error = "Only Pending items can be promoted." });

            item.Priority   = Math.Min(item.Priority + 10, 100);
            item.UpdatedUtc = DateTime.UtcNow;

            await db.SaveChangesAsync(ct);
            return Results.Ok(new { id, priority = item.Priority });
        })
        .RequireAuthorization(policy => policy.RequireRole("SuperAdmin", "Admin"));
    }
    ...
    public sealed class AdminComposeEmailRequest
    {
