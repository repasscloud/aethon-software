...
public static class CandidateEndpoints
{
    public static void MapCandidateEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // DELETE /api/v1/me/account — self-service account deletion
        // Deletes the job seeker's own account, profile, files, and all unsuccessful applications.
            ...
            CancellationToken ct) =>
        {
            ...
            if (user is null)
                return Results.NotFound(new { code = "account.not_found", message = "Account not found." });

            await DeleteJobSeekerAccountAsync(db, userManager, user, ct);

            return Results.Ok(new { message = "Your account and associated data have been deleted." });
        });
    }

    /// <summary>
    /// Deletes a job seeker account and all associated data:
    /// - Applications where the candidate was not hired
    /// - Stored files (resumes, profile picture)
    /// - Job seeker profile and all sub-records
    /// - The Identity user record
    /// The email address is removed so the user can re-register fresh in future.
    /// </summary>
    internal static async Task DeleteJobSeekerAccountAsync(
        AethonDbContext db,
        UserManager<ApplicationUser> userManager,
        ApplicationUser user,
        CancellationToken ct)
    {
        // 1. Delete unsuccessful applications (not hired)
        await db.JobApplications
            .Where(a => a.UserId == user.Id && !a.IsHired)
            .ExecuteDeleteAsync(ct);

        // 2. Null out the user reference on any hired applications (keep employer records intact)
        //    We mark these as anonymised by clearing the FK. Requires UserId to be nullable in schema,
        //    so instead we delete them too — employer retains the job record + hired status is
        //    already visible at the job level.
        await db.JobApplications
            .Where(a => a.UserId == user.Id)
            .ExecuteDeleteAsync(ct);

        // 3. Delete the job seeker profile (cascades to resumes, nationalities, languages, etc.)
        await db.JobSeekerProfiles
            .Where(p => p.UserId == user.Id)
            .ExecuteDeleteAsync(ct);

        // 4. Delete stored files owned by this user
        await db.StoredFiles
            .Where(f => f.UploadedByUserId == user.Id)
            .ExecuteDeleteAsync(ct);

        // 5. Delete the Identity user (cascades ASP.NET Identity tables)
