    ...
    public static void MapAuthEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            RegisterRequest request) =>
        {
            ...
            try
            {
                ...
                {
                    // non-fatal — user can request resend, but log so admin can diagnose
                    await systemLog.ErrorAsync("Email",
            ...
            LoginRequest request) =>
        {
            ...
            // Record last login timestamp (fire-and-forget update)
            await db.Users
                .Where(u => u.Id == user.Id)
                .ExecuteUpdateAsync(s => s.SetProperty(u => u.LastLoginUtc, DateTime.UtcNow));
            ...
            CancellationToken ct) =>
        {
            ...
            {
                // non-fatal — but log so admin can diagnose delivery issues
                await systemLog.ErrorAsync("Email",
                    $"Failed to resend verification email to {request.Email}",
                    $"UserId: {user.Id}",
                    exception: emailEx,
                    ct: ct);
            }
