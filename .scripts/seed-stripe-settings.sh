...
# seed-stripe-settings.sh
#
# Reads key=value pairs from a file and upserts each setting via:
#   PUT /api/v1/admin/settings/{key}
#
# Usage:
#   chmod +x seed-stripe-settings.sh
#   ./seed-stripe-settings.sh stripe-settings.env
#
...
import os, json
print(json.dumps({"value": os.environ["RAW_VALUE"]}))
PY
  else
...
http_status="$(printf '%s' "$raw_login_response" | tail -n1)"
login_response="$(printf '%s' "$raw_login_response" | head -n -1)"

if [[ "$http_status" != "200" ]]; then
  echo "Login failed with HTTP $http_status." >&2
